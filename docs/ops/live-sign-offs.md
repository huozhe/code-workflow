...
**So the remaining clause belongs in an in-image fixture** — and it already had one,
which this section failed to say for two months. `tests/test_adr35_kill_in_image.py::test_2_no_descendant_survives_the_kill`
has existed since #199: a stand-in CLI (`sleep 300 & sleep 300`) holding a real
descendant, `_kill_unlocked` called as the role, and the assertion made on `/proc`
**State** rather than on the pid's existence — because a SIGKILLed process is a zombie
until reaped and pid 1 reaps nothing (that is #210), so an existence check reads dead
as alive.

**What was actually missing was CI.** `pr.yml` puts a failing `docker` stub first on
PATH, so every in-image test skips there; `image.yml` ran only
`tests/test_image_agent_clis.py`. So ADR-35's kill fixture had **never run in a CI job
since the day it was written**, and a green `image` check said nothing about the
descendant clause. #244 puts it, and #233's fixture, on that job and its path filter.
A fixture no job runs is not a guard, and a DISCHARGED row pointing at one is the green
reading this ledger exists to stop. **Raised reviewing #244**, where the first version of
this row cited a new helper-level test instead — one that would have stayed green if
`_kill_unlocked` stopped calling `_killpg_as_role` at all.

**#233 is discharged against the sibling wall**, in `tests/test_image_role_privilege_boundary.py`:
`--cap-drop ALL` denies uid 0 the role's `0700` token dir, the teardown wipe now drops to
the role uid, and the fixture drives `handle_request('session.teardown')` rather than the
wipe helper — calling the helper would stay green with the call deleted from teardown.
