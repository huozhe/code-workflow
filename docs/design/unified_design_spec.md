...
| **Version** | 1.26.0 — see [Revision history](#revision-history) |
...
2. **`is_error` respawns.** After a turn whose result carries `is_error: true`, `sess.proc.pid` differs from its value before the turn, and the new process was spawned with `-c`. **Assert the pid**, not that a respawn was attempted — the pid is the only thing that proves the old stream is gone.
...
4. **The grok path is unchanged.** `_grok_turn_unlocked` with a leftover response of a *lower* `mid` still returns its own — (a) must not convert grok's working id filter into a drain-dependent path.
