...
**Do not probe the group to decide the escalation.** `killpg(pgid, 0)` from root is **EPERM**, not `ESRCH` — the permission check applies to signal 0 as well, so a root probe cannot tell "gone" from "not permitted" and reads a dead group as a live one. The escalation decision is `proc.poll()` on the direct child, which needs no permission at all; descendants are settled by the `/proc` scan. Any probe that must go over a signal goes through the helper.
