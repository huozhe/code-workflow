...
| **Version** | 1.34.0 — see [Revision history](#revision-history) |
...
**The inventory and the probe both need a seam, and one of them needs a new field.** `list_managed_containers` (`reconciler.py:85`) returns `id`, `project` and `started_at` only — no `State.Running` — and the reconciler makes no RPC call anywhere today. Add `running` from the `docker inspect` the inventory already runs (`gc.py:38` collects the same field, so this is a restatement rather than a new dependency), and inject the probe as a callable beside `list_containers` and `remove_container`, defaulting to the shared serviceable predicate. The `running` flag is not the trigger — the probe is — but without it the WARNING cannot distinguish *stopped* from *running-but-uninitialised*, and those are the two causes ADR-25 fused into one repair. A log line that cannot name which one occurred is the log line that sends the next reader to the wrong section.

On success, stamp `last_seen_at` — a bare `UPDATE`, never `upsert_runner`, which would rewrite `tier`, `endpoint` and `token` from whatever the caller happened to hold. On failure, log at WARNING with project, container id and reason, and count it in the pass report. That is the whole feature. **No schema change: the column exists, so this stays a gateway-only deploy that must not ride an image rebuild.**
...
5. One project's probe failure does not abort the pass: two projects, one refusing connections, the other still probed and stamped.
...
9. A whole-fleet reboot fixture — every runner running, every one reporting `initialized: false` — leaves every `tier` at `'hot'` and `count_hot_sessions` unchanged across the pass. The literal step 7 fails this item by construction, which is why it is stated separately from (1).
10. The default probe seam is the same predicate `_runner_reachable` uses: asserted by calling the shared function directly from both tests, not by asserting two similar behaviours.

**Deliberately out of scope.** Demotion, the idle timer, §12.2's RAM row, and eager repair. `tier='cold'` still has no production writer after this ADR. What changes is that when one arrives, GC will not delete what it stops, admission will not have been told that resident RAM was freed, and the LRU clock it reaches for will have been named as the wrong one.
