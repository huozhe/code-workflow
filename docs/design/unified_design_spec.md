...
**No restore may raise the checkbox without a second signal.** `changes.body.from` does not say *who* made the prior tick. Any composition that can raise the box — restore-up (`was is True and now is False`), B4 with `was=True` (line deleted; `set_checkbox_in_body` *inserts* a ticked line), B2 with a ticked `prev_block` (whole block deleted and re-spliced) — requires `sessions.verified_at` (ADR-16). Absent it, leave the body alone. Restoring *down* (B3, B5, B4-with-`was=False`, B2 with an unticked block) cannot forge a tick and stays payload-driven. B1 (already-checked before and after a step-refine, including a missed owner-tick delivery) is unchanged.
...
**New rule.** Any composition that can raise the checkbox requires `sessions.verified_at`. That column is written only on the owner path (`is_owner and not is_agent`), so it is the only provenance signal in the system. The spellings are one operation:

| branch | payload pair | what it writes |
|---|---|---|
...
"Restore up" and "restore to `was`" read as different operations. Under the forge they are the same operation with different spelling: an agent tick, then an agent removal, then a compose that materialises a tick no owner wrote. The first PR gated only restore-up and left B4 open; B4 is how `set_checkbox_in_body` *inserts* a ticked line that never existed on the after-image. B2 is the same with the whole block. One helper (`_can_raise_checkbox`) gates all three. Absent `verified_at`, do not PATCH: the agent already put the desired end state on GitHub, and writing nothing is the smaller action. Log the refusal at WARNING with the *branch name*, `was`, `now`, `verified_at`, and the sender.

**Unchanged.** B3, B5, B4-with-`was=False` (restore *down* cannot forge), B2 with an unticked `prev_block`, and ADR-15's two guards — fresh read, `now_current != now` abort, sanity floor, skip-if-unchanged. Those were proven correct in the same live run (two deliveries aborted with `checkbox moved since webhook`). Restoring a recorded owner tick still raises — that is #65's real case and the regression this gate must not break.
