...
| **Version** | 1.32.0 — see [Revision history](#revision-history) |
...
| Version | Date | Change |
|---|---|---|
| **1.32.0** | 2026-08-21 | **Both loop-safety questions are asked of the wrong subject** (#156, #147, ADR-32). `observed = state_changed or kind in _OBSERVED_PROGRESS_KINDS` (`design_loop.py:1011`) is computed from the delivery that *woke* the turn, so a webhook the turn itself produced cannot count: a six-minute turn that opens a Design PR scores `observed=False`, and three of them escalate a healthy session (session #84 on disk, its `paused_reason` naming claimed actions with "no matching event arrived" — the events arrived, nothing looked). Ingress is accept-and-queue and writes `deliveries` from the request path, so the mid-turn webhook is already in the table; `observed` becomes state-compared-across-dispatch **or** a progress-kind delivery inside the turn's `received_at` window, with the triggering delivery excluded by construction. `public_actions` still never reset (PR #42 B1). The `silent_turns` read-modify-write spanning the turn becomes an atomic SQL update — required before any threading work, and the reason this cannot wait behind it. Separately, `paused` (`:603`) is checked and the DEFER branch returns at `:810` **sixty-three lines before** the `TERMINAL_STATES` gate at `:873`, and the close path writes `state="CLOSED"` without clearing `paused_reason` (`:2675`, cleared only on unpause at `:3090`) — so a closed-while-paused session re-defers every delivery forever (#147: 5,918 log lines from two ids). The terminal check moves above the defer; the close clears the reason. **The quiet log is not a fix**: both of #147's deliveries are `dropped` because `agentctl` ran a manual `quarantine_deferred`, and sessions #84 and #32 still carry stale reasons today. |
...
Where a live demonstration's claim *is* an identity claim — "the Developer could not approve its own PR" — record **how the action was produced**, not only that it happened. GitHub logs the identity, not the operator, so unlike every other live claim in this project it cannot be re-checked from the database afterwards. #55's spurious approval was caught only because the Architect noticed a review under its own name that it had not written. #56's step 3 is outstanding for this reason and is unchanged by this section.
...
- **Prune dangling images built from agentd's own tags, and stopped containers carrying `label=agentd.managed=true`.** Nothing else, ever. The host is not assumed to be agentd's alone — this one runs three unrelated containers — and this is ADR-4's rule, already implemented in the reconciler's orphan inventory.
...
- **a later action on an already-indexed object** — a PR merged or synchronized, an issue reopened. Adopt the transition `fsm.transition()` would have made and log the divergence;
...
*Fixes #156 and #147.* **Both are the same shape and they are eleven lines apart, which is why they land together: `_process_one` asks a question about the delivery when it means the turn, and asks whether the session is *paused* before asking whether it is *alive*.** Neither is a bug in the thing being asked — `SilentTurnTracker` (`loop_safety.py:48`) and `route_for_recipient` (`routing.py:74`) are both correct and both faithfully answer what they are given.
...
`kind` is the kind of the delivery that *woke* this turn and `state_changed` is the FSM transition **that same delivery** caused. Both are fixed before the agent starts work, so a webhook produced *by* the turn — the whole point of the turn — cannot contribute to either term. `silent_turns` is reset in exactly two places: owner activity (`:696`) and this post-turn evaluation (`:1015`). A progress webhook arriving mid-turn reaches neither, and counts only if it later happens to trigger a turn of its own.

So the detector's model is *"progress is a property of the delivery I was woken by"*, when the question §9.3 exists to answer is *"did this turn move anything."*

**The cost, with real numbers.** An architect turn triggered by an `issue_comment` — not in `_OBSERVED_PROGRESS_KINDS` (`:137`), no FSM transition — that spends six minutes opening a Design PR scores `observed=False`. Session #151's turns ran 5 m 56 s each. With `silent_turn_limit` defaulting to 3 (`config.py:120`), three productive turns of that shape escalate a healthy session to `PAUSED_HUMAN`. Session #84 is still on disk carrying the result, and its `paused_reason` names `public_actions=['api_write','comment']` with *"no matching event arrived (P1)"* — **the events arrived; nothing ever looked for them.**

**The window is observable, and cheaply.** Ingress is accept-and-queue (ADR-10) and writes `deliveries` from the request path (`server.py:238`), independent of the drain — so a webhook produced mid-turn **is already in the table** with a `received_at` inside the turn's `[started_at, ended_at]`, even though the single drain thread is blocked in `_dispatch_turn` and will not process it until the turn ends. Binding: `observed` becomes *"the FSM state changed across the turn, **or** a delivery for this session arrived within the turn's window whose kind is in `_OBSERVED_PROGRESS_KINDS`"*. Capture `state` before dispatch and compare after; query the window by `received_at`. **The triggering delivery is excluded by construction** — its `received_at` precedes `started_at` — which is what keeps this from collapsing into "everything is progress."

**(B) The paused defer is evaluated before the session is known to be alive, and `paused_reason` outlives the close (#147).**

Two facts, and the second is why the first is unbounded:

- `paused` is computed as `state == "PAUSED_HUMAN" or bool(sess.get("paused_reason"))` (`:603`), and the DEFER branch at `:810` returns unconditionally — no age bound, no terminal check. **`TERMINAL_STATES` is checked at `:873`, sixty-three lines later**, so for a deferred delivery it never runs at all.
- The close path writes `state="CLOSED"` and **nothing else** (`:2675`). `paused_reason` is cleared only on owner unpause (`:3090`). A session that was paused and then closed keeps its reason forever.

The result is a `CLOSED` session that still reads as paused, re-deferring every delivery it owns on every 5 s drain tick, permanently. #147 measured ~24 log lines a minute and 5,918 `route defer` lines from **two** delivery ids.

**#147's own framing understates it, and the current host proves the precondition is live.** The issue says *"Low priority. No correctness impact — the session is `CLOSED` and nothing should act on it."* The churn is indeed harmless; the *state* is not, because it means the terminal gate #85 added is unreachable on the one path that most needs it. Today, with the queue empty:

```
huozhe/code-workflow#84  CLOSED  paused_reason = "stall: silent_turns — 3 consecutive agent turns…"
huozhe/code-workflow#32  CLOSED  paused_reason = "GitHub issue is closed; session is still IMPLEMENTING…"
```

**And the log being quiet today is not evidence of a fix.** Both of #147's named deliveries are now `dropped`, and the only caller of `quarantine_deferred` is `agentctl` (`src/agentctl/__main__.py:171`) — a manual backlog purge. The data was cleared by hand; the mechanism is untouched. An earlier draft of this session's handoff recorded #147 as "already fixed in practice" on the strength of `0 deferred, 0 route defer lines`, which is exactly the inference this paragraph exists to stop the next reader making.

**The decision.**

- **(a) `observed` is measured over the turn's window.** State compared across dispatch, plus a `received_at`-bounded query for progress kinds. `public_actions` remain untouched and still never reset the counter — this widens what counts as *observed*, it does not start trusting the agent (PR #42 B1).
- **(b) The `silent_turns` update becomes atomic.** Today `sess` is read before dispatch (`:797`), the counter is read from that snapshot (`:1004`) and written back after (`:1017`) — a read-modify-write spanning the whole turn. It is correct only because one thread does everything. **This is required before any threading work regardless of (a)**, and it is the reason (A) must not be deferred behind #147's threading half: a reset applied during the turn would be silently overwritten by a count computed from the stale read, in the direction that causes false escalations.
- **(c) The terminal check moves above the paused defer.** A delivery for a session in `TERMINAL_STATES` is marked `done` and never deferred. This is the ordering fix; it is the same argument ADR-30 (c) made for the spent-event gate, in the opposite direction — there the check had to sit *after* the FSM so P1 still recorded, here it must sit *before* the defer so the gate is reachable at all.
- **(d) Closing a session clears `paused_reason`.** `:2675` writes `paused_reason=None` alongside `state="CLOSED"`. Without this, (c) fixes the symptom on new closes and leaves every already-closed session on disk still reading as paused. **Existing rows are data repair and are named in acceptance, not silently migrated.**
...
- **Threading the drain.** #147 is cited elsewhere as the single-threaded-drain issue; this ADR fixes the deferral defect its title describes and does not touch concurrency. (b) is the prerequisite that makes that work safe later.
- **The `_OBSERVED_PROGRESS_KINDS` membership list.** Adding `issue_comment` would paper over (A) by making almost everything progress, which destroys the signal.
- **§9.3's other two signals** — fingerprint and zero-thread-progress. Untouched.
- **Whether a `CLOSED` session should keep any `paused_reason` for forensics.** (d) clears it because it is load-bearing control state, not a note. If the reason is worth keeping, it belongs in `escalations`, which already stores it.

**Acceptance — (1) and (5) are where today's defects are proven, and both must fail first.**

1. **A turn that produces a progress webhook mid-flight resets `silent_turns`.** Insert a `design_pr_opened` delivery with `received_at` inside the turn's window from the dispatch stub, on a turn triggered by an `issue_comment` with no FSM transition. Assert `silent_turns` is **0** after. Today it is 1. **Assert the counter, not the escalation** — a test that only checks "no escalation" passes for two turns regardless.
2. **Three such turns do not escalate.** The unit of the bug is the run, not the turn; (1) alone passes on an implementation that resets one turn in three.
3. **The triggering delivery still does not count as its own progress.** Same shape as (1) but with **no** mid-turn delivery: `silent_turns` must increment. This is the assertion that fails if the window query is written with an inclusive lower bound, which would make every turn observe itself and disable §9.3 entirely.
4. **`public_actions` still never reset.** A turn claiming `api_write` with nothing observed increments. Name the test for PR #42 B1 so the next simplification pass does not fold the two rules together.
5. **A delivery for a `CLOSED` session is marked `done`, not deferred** — even when `paused_reason` is set. Seed exactly the shape on disk today: `state="CLOSED"`, `paused_reason` non-empty. Assert the delivery status **and** that no turn was dispatched. Today it defers forever and the assertion to write is the status, since "no turn" passes against the bug.
6. **Closing a session clears `paused_reason`.** Assert the column is `NULL` after the close path, and — separately — that a session closed *without* ever having been paused is unaffected.
7. **The escalation names an examined window.** Assert the "no matching event arrived" wording appears only when the window query ran and returned nothing.
8. **Atomicity, asserted as a query and not as a race.** Two `SilentTurnTracker` updates against the same session from stale snapshots must not lose one — express it as "the write is a single SQL increment/reset, not a read-modify-write", since a threading test would be flaky and prove less.
9. **Live, before sign-off.** No unit test proves the wiring: a real turn that opens a Design PR must leave `silent_turns` at 0. Fixture proves the window logic; only a live turn proves it is connected. **Gateway-only**: no image rebuild, no `docker rm -f`; verify by daemon restart time.
10. **Data repair, recorded rather than performed by code.** Sessions #84 and #32 carry a stale `paused_reason` today. Clearing them is an owner action and must not be folded into a migration — a migration that rewrites session state is a larger decision than this ADR makes.
