...
| **Version** | 1.13.0 — see [Revision history](#revision-history) |
...
**The fourth row is the mirror, and it does not move this section's ruling.** An agent untick that has not been restored yet, plus a close, records `ABANDONED` on a session the owner verified — the same race as the third row, pointing the other way. The answer keeps the checkbox as the authority and only changes *when* it is read: hold the delivery for the same grace window, then re-read the issue and classify from that. `verified_at` is the trigger for the re-read, never the verdict (`@huozhe`'s call, 2026-08-14). Full rule: ADR-19.
