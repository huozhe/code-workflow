...
| **Version** | 1.37.0 — see [Revision history](#revision-history) |
...
| Version | Date | Change |
|---|---|---|
| **1.37.0** | 2026-08-25 | **A dropped `pull_request.synchronize` has no node id, so the sweep cannot regenerate it, and adopt would skip the review turn** (#211, ADR-37). Observed three times on session #57: `DESIGN_REWORK` stranded 26 minutes (GitHub: *failed to connect to host*); a failed `APPROVED` recovered only because the owner redelivered it; then `CODE_REWORK` lost the Developer's push, the silent-turn guard paused the session, and the later legitimate merge recorded `unauthorized Feature PR merge` because the FSM never reached `MERGING`. Cause is transport (Tailscale funnel), not HMAC or a 4xx. ADR-21 named synchronize as an adopt, which is the wrong half: `_adopt_forward` writes `state` and dispatches nothing, and `fsm.py` has no `DESIGN_REVIEW`+`design_revised` row anyway. Repair is a session head-SHA watermark plus a synthesized `pull_request.synchronize` when live head moves. Watermark advances when `_process_one` takes the event, not when the turn ends. Synth payload carries `pull_request.user.login` (the #209 hole). Unique `delivery_id` `recon:sync:<pr>:<sha>` stops a second pass re-inserting. Delayed real webhook with the same SHA is spent. Not `check_suite` (also droppable), not hook-redelivery (needs `admin:repo_hook`), not a quiet-pass `PAUSED_HUMAN` (punishes a correct wait). Gateway-only. |
...
**`turns.public_actions` is not the record, and the difference is load-bearing.** It is the obvious place to look and it is agent-authored: *"public_actions are claims (tool_use) — diagnostic, not a reset"* (`design_loop.py:1035`), *"same untrusted source as advisory `status`"* (`loop_safety.py:68`). An agent that would misuse a credential would also write that row. Only the gateway-written columns are worth cross-checking, which is why the stamp is checked against `role` and the turn window and against nothing the agent supplied.
...
**The mechanism, and every step of it is behaving as written.** `_event_kind` produces `design_revised` / `feature_revised` only for `pull_request` + `action == "synchronize"` on the tracked Design / Feature PR (`design_loop.py:3520`). `_synthesize_node` (`reconciler.py:667`) has three kinds: `comment` → `issue_comment.created`, `pull_request` → `pull_request.opened`, `review` → `pull_request_review.submitted`. There is no kind for a head-SHA change, and the PR kind is permanently `opened` because membership is per object (ADR-21): `pull_request.node_id` is one ID from opened through merged. Step 4 currently adopts `feature_merged` / `design_merged` only (`reconciler.py:601–604`). Six consecutive passes on #57 reported `nodes=6 synthesized=0`. By the reconciler's own model there was nothing to do.
...
| **B** | Adopt the FSM transition (ADR-21 as written) | A `_adopt_forward(sess, "design_revised")` next to the merge adopts. No schema | Low | **Rejected.** `_adopt_forward` (`reconciler.py:642`) writes `state` and does not dispatch. Reviewer is never asked. `fsm.transition("DESIGN_REVIEW", "design_revised")` is `None` anyway, so a second lost push while already in review is a no-op even as an adopt. This is how the issue's "looks healthy" row is manufactured in code |
...
**(a″) A real `synchronize` whose payload SHA equals the stored watermark is spent.** Mark `done`, no turn. Place it **after** the FSM would have run for a *new* SHA and **as a sibling of ADR-30 (c)**, not inside the author-drop at `:443`: `synchronize` is an FSM kind and P1 must still record a first sighting; a duplicate of a SHA we already took is the delayed webhook that lost the race to the synth. If the payload SHA is *newer*, take it and advance. If stored is NULL, take it and stamp. Fail open if `head.sha` is missing from the payload — a payload without a SHA cannot be the cue this ADR exists to recover, and dropping it would hide a fixture bug as a fix.

This is the delayed-webhook race: synth drains first, stamps, real GUID arrives 101 s later (the observed redelivery latency), spent-SHA fires, no second turn. Real arrives first, stamps, synth insert is a new row whose payload SHA then matches, spent-SHA fires on drain. Either order, one turn.
...
- `sender.login` = that same PR author (synchronize is always author-sent; ADR-30 (a″) excludes `pull_request` from the author drop *because* of this, and a synth that omitted `user.login` would not hit the drop — it would hit #209's hole instead, routing to the counterpart as a comment)
...
- **State `DESIGN_REWORK` or `CODE_REWORK`, watermark NULL:** synthesize. The only FSM exit is `*_revised`. The live instances were this row. Residual: a session correctly waiting for a push (live head still the `CHANGES_REQUESTED` commit) gets one extra review of that same head. One turn, then a real verdict. Named, cheaper than sealing #57.
...
**(d) Do not synthesize `push`.** It is not an FSM kind. ADR-30 (a″) already excluded it from the author rule for the same reason `synchronize` is kept: the author is the only pusher. One event, the one `_event_kind` already maps.
...
5. **A real `synchronize` for a *newer* SHA still routes, still yields `design_revised`, still dispatches one Developer turn.** ADR-30 (a″) paired with this so a spent-SHA gate written too wide does not kill rework. Same module.
