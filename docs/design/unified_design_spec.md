...
| **Version** | 1.27.0 — see [Revision history](#revision-history) |
...
| `pull_request_review` (any action), `pull_request_review_comment`, or `issue_comment` **on a PR**, whose `sender.login` is that PR's own author | **Drop** — an agent's own PR is not the counterpart's cue (ADR-30, #173) |
| A PR-scoped event, other than the merge itself, whose PR is `merged`/`closed` when the delivery drains | **Drop after the FSM records it** — the obligation is spent (ADR-30, #173) |
...
**(a) The rule: an event a PR's own author sends about that PR is not the counterpart's cue.** Drop, per recipient, when `sender.login` equals the PR's author for exactly three events:
...
**(c) The spent delivery — #173's fourth trigger, and it belongs at a different site for a reason that is not obvious.** A delivery deferred through `PAUSED_HUMAN` drains on resume against a state that has moved past it: #169's `DESIGN_REWORK` on review `4985744265` and `DESIGN_APPROVED` on `4985859926` both arrived after PR #170 was merged at 18:10:02, and the `synchronize` at 18:11:43 arrived after the head branch was gone. Neither obligation could be performed twice, and the only honest answer to either is the no-op that feeds the next pause. **The naive fix — drop it beside (a) at `:443` — is wrong**, and this is the correction that placing the code produces: all three of those events are FSM kinds (`design_changes_requested`, `design_approved`, `design_revised`), so a pre-FSM drop discards a transition P1 exists to record. The correct site is the one the codebase already uses for exactly this shape: the terminal-state gate at `:775–784`, whose own comment reads *"after FSM (P1 still records a late event), before stall / DROP / dispatch."* Binding: **after the FSM block, a PR-scoped event whose PR is `merged` or `closed` at drain time, and whose kind is not itself `design_merged` / `feature_merged`, records its transition and dispatches no turn.** One REST read (`gh api repos/{repo}/pulls/{n}` — no `read:org`, no GraphQL, per §5.1's scope reality), taken only for a delivery that was actually parked, not on the hot path.
...
6. **An author-sent review whose state is `approved` or `changes_requested` routes and logs WARNING.** Unreachable through GitHub today; the test is what stops the drop eating a verdict if that ever changes.
7. **(c): a review deferred past its PR's merge records the FSM transition and dispatches no turn.** Assert **both** halves. A test asserting only "no turn" passes against a version that also skips the FSM, which is P1's regression and the reason (c) is not at `:443`.
