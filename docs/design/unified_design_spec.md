...
| **Version** | 1.30.0 — see [Revision history](#revision-history) |
...
| Version | Date | Change |
|---|---|---|
| **1.30.0** | 2026-08-21 | **Session worktrees are based on the remote-tracking ref, and the gateway fetches** (#171, ADR-31). Two independent defects: `worktree_add`'s `base_ref` defaults to `HEAD` — the clone's local default branch, frozen at clone time and on this host **forked**, no common ancestor, 60 commits down a squashed-away branch — and there is no `git fetch` anywhere in `src/agentd/`. **#171 ranks them backwards.** `origin/<default>` in the clone is kept current as a side effect of *agents* fetching inside worktrees, which share the clone's `.git`, so basing on the remote ref alone would have fixed #169 with no gateway fetch; fetching alone updates a ref nothing reads. Live on #169: the architect detected its 60-commit-stale base and reset unprompted, while the Developer reviewed Design PR #170 out of a worktree 67 commits back — the write path fails loudly, the read path silently. (a) resolve the default branch from `refs/remotes/origin/HEAD`, pass `base_ref=origin/<default>` at both `supervisor.py:827–828` sites, WARNING on the `main` fallback; (b) `git fetch origin --prune` plus `remote set-head origin --auto` inside `ensure_shared_clone`'s existing per-repo lock; (b′) a failed fetch warns and proceeds, safe only because (a) makes the fallback stale rather than forked; (c) an existing branch is **never** re-based — it carries the agent's work — but a stale resume logs its distance, correcting a checklist item that reads as asking for an automatic rebase; (d) the clone's local default branch is left alone and becomes inert, its divergence logged so `git -C <clone> log main` stops misleading readers. Blocks #173's live sign-off: the clone sits at `85107b4`, four commits behind, so a session opened today would review ADR-30 out of a tree without it. |
...
- **(a) Session worktrees are created from the remote-tracking ref.** Resolve the default branch from `refs/remotes/origin/HEAD` — `git clone` sets it, and it reads `refs/remotes/origin/main` on this host today — short-name it, and pass `base_ref=f"origin/{default}"` at both `supervisor.py:827–828` call sites. `worktree_add` already takes the parameter and nothing has ever passed it. Fall back to `main` **only** when `origin/HEAD` is absent, and log at WARNING when falling back, because a silent default is how the current bug reads as working.
- **(b) `ensure_shared_clone` fetches, inside the per-repo lock it already holds.** `git fetch origin --prune`, plus `git remote set-head origin --auto` so a renamed upstream default is followed rather than frozen at clone time. The existing `_lock_for(path)` serialises this against concurrent `worktree_add`, which is what we want; the cost is one fetch per `ensure_session`, against a session that runs for minutes.
- **(b′) A failed fetch is a WARNING, not a failure.** It does not refuse the session. This is safe *because of* (a): with the base on `origin/<default>`, a fetch that could not run leaves a base at worst as stale as the last successful one, and **never forked** — the fork came from the local branch, which (a) stops reading. Refusing to start a session on a transient network failure is the worse trade, and it would make the gateway's liveness depend on GitHub's.
- **(c) An existing branch is never re-based, and this contradicts how #171's checklist reads.** `worktree_add`'s existing-branch path (`gitops.py:173–175`) checks the branch out as-is, and that is **correct**: on a resume that branch carries the agent's committed work, and silently rebasing or resetting it destroys work to fix a base. The checklist item — *"a session resumed onto a stale pre-existing branch must not silently keep the stale base"* — is right that the situation must not be silent, and wrong if read as asking for an automatic rebase. Binding: on the existing-branch path, log at WARNING with `git rev-list --count <branch>..origin/<default>` when that count is non-zero. Visible, and the agent already recovers when it can see the problem — #169 proves that.
- **(d) The clone's local default branch is left exactly as it is, and after (a) nothing reads it.** Deliberately not aligned to `origin/<default>`: it is the checked-out branch of the clone **root's** working tree, whose cleanliness the gateway does not own, and `--ff-only` is impossible against a forked history anyway (the repair below needs `--hard`). Log its divergence once at ensure time, at WARNING when non-zero, so a human running `git -C <clone> log main` is not misled — **that misleading read is how this survived nine milestones.** Answering #171's open question directly: the local branch is not a working ref, it is a clone artefact, and this ADR makes it inert rather than correct.
...
**Repairing this host, which must happen before #173's live sign-off.** `merge --ff-only` fails — no common ancestor — so the local branch must be forced:
...
git -C $C reset --hard origin/main
```

Safe because agentd mutates the clone only through session worktrees. Verified clean on this host on 2026-08-21. This does **not** repair worktrees already on the stale base.

**Acceptance — (1) is where today's defect is proven, and it must fail first.**

1. **A session created against a clone whose local default branch is behind `origin/<default>` bases its branches on the remote ref.** Build the fixture by committing to the source repo after the clone exists, then assert `git rev-list --count <new-branch>..origin/<default>` is **0** for both roles. Non-zero today. **Assert on the base, never that a worktree was created** — the existing `test_worktree_add_under_two_seconds_warm` asserts a worktree exists and passes against the bug.
...
3. **`ensure_shared_clone` fetches an existing clone.** Commit to the source after cloning; assert `origin/<default>` in the clone moves without any worktree being created. The fixture repo is a local path, so this stays offline.
4. **A failed fetch does not refuse the session.** Point the remote at a nonexistent path, assert the session is still created, the WARNING is logged, and the base is still `origin/<default>` — the whole argument for (b′) is that (a) holds without (b).
5. **The default branch is resolved, not assumed.** A source repo whose default branch is not `main`; assert the base ref names that branch. Assert separately that a clone with no `refs/remotes/origin/HEAD` falls back to `main` **and logs the fallback**.
6. **A resumed branch keeps its commits.** Existing branch with a commit on it, clone behind: assert the commit is still reachable from the worktree HEAD after `worktree_add`, and that the staleness WARNING fired. A test asserting only the warning would pass against an implementation that rebased and destroyed the work.
7. **Live, after deploy.** Run the repair above, then create a session and assert `git rev-list --count HEAD..origin/<default>` is 0 in **both** role worktrees with no agent action — #171's stated exit condition, and the only assertion that covers `ensure_session` end to end. **Gateway-only change**: no image rebuild, no `docker rm -f`; verify by daemon restart time.
