...
| **[41](ADR/adr-41-agentctl-sessions-live.md)** | **`agentctl sessions --live` — the set from `list_nonterminal_sessions`, the fields from the row it does not carry.** Membership is the sweep's own method, called rather than re-expressed; ordering, ASCII output and the absent-PR placeholder are CLI decisions, each one measured. Names why `--live` is not a drop-in for the runbook's "is anything live" check: `TEARDOWN` dispatches agent turns. Fixes #248. |
---
