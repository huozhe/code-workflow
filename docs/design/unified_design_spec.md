...
| **Version** | 1.26.1 — see [Revision history](#revision-history) |
...
| Version | Date | Change |
|---|---|---|
| **1.26.1** | 2026-08-20 | **ADR-29 implementation binding** (#169). The decisions stand; five corrections found placing the code are recorded as an addendum to ADR-29, and the implementation contract — placement, sequencing, and the test plan against acceptance (1)–(8) — moves to [`rfcs/169-adr-29-held-pipe-turn-boundary.md`](rfcs/169-adr-29-held-pipe-turn-boundary.md). Load-bearing among them: **(d) is a no-op as written**, because `_run_teardown_turns` resets `_delivery_attempts` (`design_loop.py:2664`) before the open-artifact check reaches `_teardown_retry_or_give_up`, so the delivery would defer at attempt 1 forever and never escalate; and acceptance (7)'s live criterion is zero discarded frames of `type == "result"`, since a startup `system`/`init` frame is a legitimate first-drain discard. |
...
**Addendum (#169, spec 1.26.1) — implementation binding, and five corrections found in the tree.** ADR-29's decisions are unchanged; these are the places where writing the code contradicts the text, recorded here because an implementer meets them and a reader of this ADR would otherwise be misled. Full contract: [`rfcs/169-adr-29-held-pipe-turn-boundary.md`](rfcs/169-adr-29-held-pipe-turn-boundary.md).

- **(d) is inert as specified.** `_run_teardown_turns` pops `_delivery_attempts` on its success path (`design_loop.py:2664`) before control reaches the open-artifact check at `:2257`, so routing that check through `_teardown_retry_or_give_up` re-enters at **attempt 1 on every redelivery**: the delivery defers forever, never escalates, and re-dispatches two teardown turns per dispatcher tick against a closed issue. The counter is per delivery and must survive until the delivery finishes — the pop at `:2664` goes, and the clear moves to the success path before the final `set_delivery_status(..., "done")`. Acceptance (6) is therefore asserted **across** passes, not after one; a single-pass assertion passes against the broken version too.
- **(a)'s WARNING needs a `turn_id` the runner never passes down.** `LiveCliSession.turn` (`cli_session.py:493`) has no turn id; `exec_turn_as_role` computes one at `turn.py:528`, before it calls `_live_result`. It is threaded through as a keyword-only parameter defaulting to `None` (rendered `turn_id=unknown`), not stashed on the session — per-turn state on the session object is the failure mode this ADR exists to close.
- **(a) and (b) invalidate the CLI-session fixtures, which is the point.** The suite seeds `_stdout_q` *before* calling `turn()` (18 direct assignments across `test_cli_session.py`, `test_public_actions.py`, `test_turn_privilege.py`). Every seeded frame is stale by (a)'s definition, so those turns block to `deadline_s`; frame delivery moves onto the `stdin.write` side effect, which is the same causality the real CLI has and the only shape in which acceptance (1) and (4) can both be written. Separately, the five `is_error` tests now reach (b)'s respawn and must patch or assert `_spawn_unlocked` rather than spawn a real `claude` in the unit suite.
