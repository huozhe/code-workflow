...
Place the gate **as a sibling of ADR-30 (c)**, sharing its GET, not inside the author-drop at `:443`. `synchronize` is an FSM kind; P1 must still record a first *take*. A payload without `head.sha` fails open (cannot be this cue; dropping it would hide a fixture bug as a fix). A failed GET fails open on the stale bullet only — cannot prove stale, fall through to the duplicate check. Named residual: a late `X` during an API outage still dispatches. Same posture as ADR-30 (c) on a failed fetch.
