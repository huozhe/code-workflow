...
| **Version** | 1.23.0 — see [Revision history](#revision-history) |
...
- **A module-level in-flight turn-id set in `design_loop.py`, beside `_role_busy_until` (`design_loop.py:136`):** `_inflight_turn_ids: set[str] = set()`. `_dispatch_turn` adds `turn_id` immediately after `insert_turn` (`design_loop.py:1009`, before the RPC) and discards it on every exit thereafter — a `try/finally` wrapping the rest of the function is sufficient, since there is exactly one early return after that point (the RPC's `except` block) plus the function's own final return; no other exit exists. Read and written from two threads (dispatcher, reconciler) with no lock, the same posture `_role_busy_until` already takes on this file today — single-`set`-membership and single-`set`-mutation calls are atomic under the GIL, and neither field needs a read-modify-write that spans the boundary between them.
...
**Acceptance.** (1) A turn present in `_inflight_turn_ids` is never resumed or retired by `_apply_open_turn`, regardless of session state or age — unit-testable by driving the reconciler against a fake store with the id pre-seeded. (2) A turn absent from the set, in a `NON_RUNNING_STATES` session, retires at any age — unchanged from today. (3) A turn absent from the set, session running, younger than `resume_max_age_s` — no longer resumed; this is today's defect, reproduced first, then fixed. (4) `finish_turn` on a row whose `ended_at` is already set is a no-op that logs the discarded status, not a silent overwrite — testable directly against `Store`, no dispatcher involved. (5) **The set's central claim — correct behavior after a restart — cannot be proven by unit test**, because the set is in-memory and empty at every process start by construction; unit tests can only prove live turns are skipped, never that orphan detection still fires. This needs a live run: dispatch a turn, `kill -9` the gateway mid-turn, restart, confirm the reconciler still resumes (or retires, if past `resume_max_age_s`) the orphaned turn. §18's existing "kill -9 the gateway mid-turn" chaos row covers the scenario; it gains this as an explicit assertion. Green unit tests alone are not sufficient sign-off for this ADR — record that in the verification protocol before implementation is called done.
