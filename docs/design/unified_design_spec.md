...
| **Version** | 1.21.0 — see [Revision history](#revision-history) |
...
**Key on Docker, not on the tier column.** `tier` is a cache in the same sense the `sessions` counters are: a container stops without agentd's knowledge on a host reboot, an OrbStack restart, or a hand `docker stop`. Keying promotion on `tier == 'cold'` recovers only the demotions agentd itself performed — and today it performs none — while trusting a column nothing currently writes. The condition is therefore *the container exists and is not running*, which recovers all three causes and needs no column to be accurate.
...
**Acceptance — the negatives carry the weight.** (1) `demote_cold(project)` then a turn: the **same `container_id`** is running and `tier='hot'`. Asserting only that the turn ran passes on the recreate path too, so the container id is the assertion. (2) After promotion, `runners.endpoint` equals the port Docker reports, not the stored one. (3) A stopped container whose image is not `IMAGE` is recreated, not started. (4) At `max_hot_containers`, promotion raises `CapacityRefusal` **and the container still exists** — nothing is removed on the refusal path.
