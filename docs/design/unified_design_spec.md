...
**The config gap made the bug bigger than one branch.** `docs/ops/m4-a-branch-protection.md` records that `main`'s ruleset carries no `required_status_checks` rule — collaborators cannot add one — so `repos.<repo>.required_checks` in `~/.agentd/config.yaml` is the only source, and it is unset. `not wanted` (no checks configured) is one of the two ways into the always-transient path, so on this repo, today, *every* `blocked` observation retries five times before escalating, independent of cause. #144 named `[pytest, lint, types]` for a future ruleset rule; host config and the ruleset stay unwired until that list is applied. The ruleset does carry `required_review_thread_resolution: true`, which is a cause retrying cannot fix: an addressed-but-unresolved inline thread holds a PR at `blocked` until someone resolves it, not until some amount of time passes.
