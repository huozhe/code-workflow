...
- **An agent's own PR-scoped events.** `pull_request_review` (any action), `pull_request_review_comment`, and `issue_comment` on a PR are dropped when `sender.login` is that PR's own author, beside the review-part drop and before budget, `_event_kind`, the FSM, and recipient selection. Being ahead of the `PAUSED_*` defer is the point, not an accident: replies parked through a pause drain on unpause and would otherwise become turns then.
