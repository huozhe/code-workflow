...
| **Version** | 1.35.0 — see [Revision history](#revision-history) |
...
**The reaper must not use a blanket `waitpid(-1)`, or it breaks the other role.** The runner holds one `Popen` per role. `os.waitpid(-1, WNOHANG)` consumes whichever child is reapable — including the sibling role's CLI — after which that `Popen`'s `poll()` never returns its status and its `wait()` raises `ChildProcessError`. A cleanup that introduces a cross-role failure is worse than the leak it removes. Peek without consuming — `os.waitid(os.P_ALL, os.WNOHANG | os.WNOWAIT)` — and reap with `os.waitpid(pid, os.WNOHANG)` only when that pid is not one the runner tracks. Run it after each kill and before each spawn; `--pids-limit 1024` bounds the worst case but is not the design.
...
6. Zero `State: Z` processes under pid 1 after a kill.
7. **The reaper does not steal:** with both roles' CLIs alive, kill role A, then complete a turn on role B — B's `Popen.poll()`/`wait()` still return B's own status. This is the item that fails a blanket `waitpid(-1)`.
