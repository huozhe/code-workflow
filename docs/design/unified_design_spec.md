...
3. **`ensure_shared_clone` fetches an existing clone.** Commit to the source after cloning; assert `origin/<default>` in the clone moves without any worktree being created. The fixture repo is a local path, so this stays offline — **and that is also this suite's blind spot, recorded here so it is not mistaken for coverage.** A `--filter=blob:none` clone over a local path silently keeps every blob (`warning: filtering not recognized by server, ignoring`; `rev-list --missing=print` reports **0** missing, against **130** on the live clone). No offline fixture is a real promisor repo, so nothing in this suite can exercise the lazy-fetch hazard (d) is written against. It is an acceptance (9) observation, not a unit test.
