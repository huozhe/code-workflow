...
| **Version** | 1.15.0 — see [Revision history](#revision-history) |
...
**Prefer synthesis over adoption.** Where an event can be synthesized, let it drain through the normal handlers — the same code paths, the same routing, the same FSM. Derive-and-adopt is the fallback for divergences synthesis must not create, which after the rules above means: a closed issue (escalate, never a synthetic `issues.closed`, which would trigger teardown), and body drift.

**Fold in #110.** One INFO per pass with counts — nodes seen, synthesized, capped, divergences adopted, escalations raised — because M6's exit condition is "account for every delivery" and a component that logs only its writes cannot be read that way. M6-1a ships silent on a healthy host; the sweep must not.

**Acceptance.** #32 converges in the sense this half can: reported, its closed-live divergence escalated, nothing classified. Plus the offline test — stop the daemon, post two comments, restart — which must produce exactly one `recon:` delivery per comment, and **zero** on a second run.
