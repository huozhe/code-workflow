...
**(f) ~~One call site sets the clock~~ Two call sites set the clock (amended, #230): the `RouteAction.DEFER` branch, and `role held`.** `Store.defer_delivery(delivery_id, retry_after_s)` increments `defer_count` and writes `next_attempt_at = now + retry_after_s`; the DEFER branch passes `min(5 * 2 ** defer_count, 300)` computed from the row's `defer_count` **as read, before the setter increments it** — the first defer reads 0 and waits 5 s. Read it after and every wait doubles: 10, 20, 40, and the "unchanged first retry" below is false. First retry 5 s (unchanged), then 10, 20, 40, 80, 160, and 300 s thereafter — the ceiling is `RECONCILE_INTERVAL_S`, so nothing waits longer than a reconcile pass already does. #57's 4,466 lines become **79**.

**(f′) `role held` is the third deferred-return site this ADR did not name, and it takes the exact time rather than the backoff (#230).** Measured on session #159: `role held` logged **2,065** identical lines for one delivery over 3h13m at a flat ~6 s, beside a `DEFER` delivery on the same session doing 4 in the first 40 s and holding the 300 s ceiling. This ADR reasoned about the two *other* deferred-return sites — the merge-auth transient path, which has `_MERGE_AUTH_MAX_ATTEMPTS = 5`, and the capacity re-defer on resume — and left this one with no clock and no cap.

Backoff is the wrong instrument here because **this site already knows the answer**: `_role_busy_until` is the exact moment the role frees up, and the log line prints it as `retry_in`. So it calls `Store.defer_delivery_until(delivery_id, int(held_until))` — a set-to-absolute variant, because `defer_delivery` computes `now + backoff(defer_count)` in SQL and would discard a deadline the caller holds. One attempt when the hold expires, not 2,065 guesses, and no ceiling needed for correctness.

**The clock is persisted and the hold is not, so startup must clear it.** `_role_busy_until` is an in-memory dict and says so at `session_loop.py:271` — *"in-memory gate; forgotten on daemon restart"* — while `next_attempt_at` is a column. Writing a 4-hour hold and then restarting a minute later would leave the delivery parked for the remainder of a deadline nothing is enforcing any more: **2,065 noisy lines traded for one silently stranded delivery**, which is a worse defect than the one being fixed and the same class as #211 and #229. `clear_deferred_clocks()` (g) already exists and already runs on owner unpause; it now also runs at startup, where the in-memory holds are empty too, so the two cannot disagree. The cost is one extra attempt per deferred row per restart, which is the pre-ADR-38 behaviour for exactly one tick.

**Unchanged:** the hold's own log line still emits once per attempt, so a long hold stays visible — the fix is fewer attempts, not a quieter log — and `role held` still makes **no stall observation** (`session_loop.py:1034–1038`), which is an unrelated property that a wider change would have broken.
