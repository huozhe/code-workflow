...
2. `transition(state, "<half>_pr_opened")` is not `None` — `PLANNING` for the design half; `IMPLEMENTING` or `AWAITING_VERIFICATION` for the code half (`fsm.py:67`, `:75`, `:88`);
...
**(d) The synthesized envelope is `pull_request.opened` and it must carry `head.sha`.** `_synthesize_node`'s existing `pull_request` branch (`reconciler.py:825`) writes `"head": {"ref": …}` and no SHA. That is harmless today because the branch is only reachable for a PR the columns already name; it is **not** harmless here. Without `head.sha` the take stamps nothing (`session_loop.py:994` is guarded on `payload_sha`), the watermark stays NULL, and ADR-37 (c)'s backfill arms then decide from NULL on the next pass — recovering the strand and re-arming a different one. Add the field to that branch.
...
- `pull_request.user.login` = the PR author, and `sender.login` = the same. #209's hole: an envelope missing `pull_request.user` short-circuits ADR-30's author-sent guard and routes the event as a comment to the counterpart. The discovery GET returns the PR object; take the author from it, not from anywhere else
- `repository.full_name`

**(e) Idempotency: `recon:open:<pr>:<sha>`, `INSERT OR IGNORE`, and — unlike ADR-37 (a′) — a `delivery_nodes` row.** Count `synthesized` only when the insert returns True, as ADR-37 (a′) requires.

The `delivery_nodes` decision inverts ADR-37 (a′) and the reason it inverts is worth stating, because getting it wrong produces a *second review turn on a recovery that looked complete*. ADR-37 declined the row because `recon:sync:<pr>:<sha>` is not a node id and writing the PR's node id against it would make a later real `synchronize`'s membership check meaningless. **Here the synth *is* that node's `opened`** — the same row a real ingested `opened` would have written. And it is required: the moment the synth drains and the column fills, `fetch_session_snapshot` starts returning that PR as a node, and ADR-21's set-diff would synthesize `recon:<node_id>` — a second `pull_request.opened` — because `has_delivery_node` has never seen it. Nothing is lost by writing it: membership is per object (ADR-21), so a later action on that PR node was never recoverable by the set-diff anyway. That is the whole reason ADR-37 exists.

Write the row **in the same transaction as the delivery** and only when the insert returned True — ADR-21's constraint, for its original reason: a node row without its delivery re-emits the same node every pass forever, and a delivery without its node row is this ADR's double-turn.
...
- ADR-21's "two things this half does not recover": the untracked-PR bullet now points here.
...
7. **The same snapshot a second time synthesizes zero.** `INSERT OR IGNORE`. A test that checks only pass 1 passes against a version that re-enqueues every five minutes.
...
9. **Predicate 2, both arms.** An untracked open role-branch PR on a `PAUSED_HUMAN` session synthesizes nothing **and issues no discovery GET**; restore the session to `PLANNING` and the next pass synthesizes exactly one. A version without the state predicate burns a turn per push into a state that has no transition for the event.
...
11. **A tracked half issues no discovery GET.** Assert the fetch callable is not invoked when `design_pr` is set. Cost is the reason predicate 1 is first; a test asserting only "no synth" passes against a version that GETs every session every pass.
